...
int32_t g_heart_rate;  // 定义心率
float g_SpO2;          // 定义血氧饱和度
...
void blood_data_translate(void)
{
    ...
    // 读取峰值点的横坐标  结果的物理意义为
    int32_t s1_max_index = find_max_num_index(s1, 60);
    int32_t s2_max_index = find_max_num_index(s2, 60);
...
void blood_loop(void)
{
    ...
    g_SpO2 = (g_SpO2 > 99.99) ? 99.99 : g_SpO2;

    // printf("心率%3d/min; 血氧%2d%%\n", g_heart_rate, (int32_t)g_SpO2);
}
